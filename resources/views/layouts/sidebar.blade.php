<ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">

  <!-- Sidebar - Brand -->
  <a class="sidebar-brand d-flex align-items-center justify-content-center" href="index.html">
    <div class="sidebar-brand-icon rotate-n-15">
      <i class="fas fa-laugh-wink"></i>
    </div>
    <div class="sidebar-brand-text mx-3">Apotik<sup>2</sup></div>
  </a>

  <!-- Divider -->
  <hr class="sidebar-divider my-0">

  <!-- Nav Item - Dashboard -->
  <li class="nav-item">
    <a class="nav-link" href="{{ route('dashboard') }}">
      <i class="fas fa-fw fa-tachometer-alt"></i>
      <span>Dashboard</span></a>
  </li>

  <!-- Nav Item - Obat -->
  <li class="nav-item">
    <a class="nav-link" href="{{ route('obat') }}">
      <i class="fas fa-fw fa-pills"></i>
      <span>Obat</span></a>
  </li>

  <!-- Nav Item - Supplier -->
  <li class="nav-item">
    <a class="nav-link" href="{{ route('supplier') }}">
      <i class="fas fa-fw fa-truck"></i>
      <span>Supplier</span></a>
  </li>

  <!-- Nav Item - Pelanggan -->
  <li class="nav-item">
    <a class="nav-link" href="{{ route('pelanggan') }}">
      <i class="fas fa-fw fa-users"></i>
      <span>Pelanggan</span></a>
  </li>

  <!-- Nav Item - Karyawan -->
  <li class="nav-item">
    <a class="nav-link" href="{{ route('karyawan') }}">
      <i class="fas fa-fw fa-id-badge"></i>
      <span>Karyawan</span></a>
  </li>

  <!-- Nav Item - Faktur Supply -->
  <li class="nav-item">
    <a class="nav-link" href="{{ route('faktur.index') }}">
      <i class="fas fa-fw fa-file-invoice"></i>
      <span>Faktur Supply</span></a>
  </li>

  <!-- Nav Item - Faktur Penjualan (Menambahkan menu Faktur Penjualan) -->
  <li class="nav-item">
    <a class="nav-link" href="{{ route('faktur_penjualan.index') }}">
      <i class="fas fa-fw fa-file-invoice-dollar"></i>
      <span>Faktur Penjualan</span></a>
  </li>

  <!-- Nav Item - Profile -->
  <li class="nav-item">
    <a class="nav-link" href="/profile">
      <i class="fas fa-fw fa-user"></i>
      <span>Profile</span></a>
  </li>

  <!-- Divider -->
  <hr class="sidebar-divider d-none d-md-block">

  <!-- Sidebar Toggler (Sidebar) -->
  <div class="text-center d-none d-md-inline">
    <button class="rounded-circle border-0" id="sidebarToggle"></button>
  </div>

</ul>
